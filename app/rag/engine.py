    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not retrieved_nodes:
                logger.warning("No documents retrieved", question=condensed_question)
            ...
            valid_nodes = [node for node in retrieved_nodes if hasattr(node, 'text') and node.text is not None]
            
            # If we don't have any valid nodes after filtering, handle gracefully
            if not valid_nodes:
                logger.warning("No valid nodes after filtering", question=condensed_question)
                response_text = LlamaIndexSettings.llm.complete(
                    self.no_context_prompt.format(question=condensed_question)
                ).text
                self.memory.add_exchange(question, response_text)
